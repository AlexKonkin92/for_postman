...
#CMD kinit -kt /etc/krb5.keytab -c admin@KS.WORKS -V && gunicorn --bind 0.0.0.0:8000 test_flask:app && cat /etc/resolv.conf && nslookup freeipa-dev.ks.works 
#CMD kinit -kt /etc/krb5.keytab host/freeipa-dev.ks.works@KS.WORKS && gunicorn --bind 0.0.0.0:8000 test_flask:app
CMD kinit -kt /etc/krb5.keytab host/freeipa-dev.ks.works@KS.WORKS -V && gunicorn --bind 0.0.0.0:8000 test_flask:app last
